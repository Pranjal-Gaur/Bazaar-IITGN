...
export async function POST(req: NextRequest) {
  ...
  const offer = await Offer.create({
    ...
    seller: {
      id: listing.seller.email, // will be resolved to ObjectId in full app
  ...
  await Message.create({
    ...
    content: `💬 ${session.user.name} made an offer of ₹${proposedPrice.toLocaleString()}${message ? `: "${message}"` : ''}`,
    type: 'offer',
    offerId: offer._id,
  });
